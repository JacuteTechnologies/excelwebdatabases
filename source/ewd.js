...
function ewdRequest(mode, document, multi, cell, row, column, data) {
    ...
    if (rc && c) {
        console.error(
            'Error 9111: Both cell & row/column are set. The library is unable to determine whether the request is namedEWD or cellEWD. []'
        )
            ...
            if (mode == 'r') {
                if (typeof document !== 'undefined') {
                    if (c && multi) {
                        ...
                        console.error(
                            'Error 9116: Multi not specified. [boolean]'
                        )
                    ...
                    xhttp.onreadystatechange = function () {
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            console.error(
                                'Error 9117: Request failed: Server responded with ' +
                                    this.status
                            )
                            return (
                                'Error 9117: Request failed: Server responded with ' +
                    ...
                    console.error(
                        'Error 9115: Document not specified. [name.xlsx]'
                    )
                ...
                if (typeof document !== 'undefined') {
                    if (typeof data !== 'undefined') {
                        ...
                        console.error(
                            'Error 9118: Data for write request not specified.'
                        )
                ...
                console.error(
                    'Error 9114: Mode is invalid. Mode must be [r/w].'
                )
                return 'Error 9114: Mode is invalid. Mode must be [r/w].'
            }
        } else {
            console.error(
                'Error 9113: Mode is not set. Mode is required. [r/w]'
            )
            return 'Error 9113: Mode is not set. Mode is required. [r/w]'
        }
    } else {
        console.error(
            'Error 9112: Neither cell nor row/column are set. Cell or row/column is required. [cell/names]'
        )
